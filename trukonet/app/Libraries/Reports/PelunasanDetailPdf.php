...
class PelunasanDetailPdf extends MY_FPDF {
...
//        $this->Line(5, $this->GetY(), 292, $this->GetY());
        $this->SetFont('arial', 'B', 12);
        $this->SetX(10);
        $this->Cell(190, 5, 'Tanggal Pelunasan : ' . $this->dataheader[0], 0, 0, 'C', false);
        $this->SetTextColor(0, 0, 0);
        $this->SetY(28);
...
//    $this->Ln();
$header  = array('No','IdPelanggan', 'Nama','Paket','THBL Tag','Hari' ,'Tagihan', 'Bi Admin', 'Total Tagihan');
...
$this->SetX(10);
        ...
        $this->Cell($w[1], $hcell, $header[1], 'TB', 0, 'L', true);
        $this->Cell($w[2], $hcell, $header[2], 'TB', 0, 'C', true);
        $this->Cell($w[3], $hcell, $header[3], 'TB', 0, 'C', true);
        $this->Cell($w[4], $hcell, $header[4], 'TB', 0, 'C', true);
        $this->Cell($w[5], $hcell, $header[5], 'TB', 0, 'C', true);
        $this->Cell($w[6], $hcell, $header[6], 'TB', 0, 'R', true);
        $this->Cell($w[7], $hcell, $header[7], 'TB', 0, 'R', true);
        $this->Cell($w[8], $hcell, $header[8], 'TB', 0, 'R', true);
...
//        $this->Ln();
    ...
    function create_pdf($data) {
        $header  =  array('No','IdPelanggan', 'Nama','Paket','THBLTag','Hari' ,'Tagihan', 'Bi Admin', 'Total Tagihan');
        $this->AddPage();
//        $this->SetAutoPageBreak(true, 18);
        ...
        $this->SetFont('arial', '', 8);
        $this->lineh = 5;
        $w = array(10,25,35,15,15, 10, 25, 20, 25);
        $a = array('R','C', 'L', 'C', 'C', 'C', 'R', 'R', 'R');
        $this->SetWidths($w);
//        $this->SetAligns($a);
